...
nq.plot(x1, [y1, y2, y3], title='Multiple plots together') # Plot multiple sets in one plot.
nq.plot(x1, [y1, y2, y3], together=False, title='Subplot') # Plot multiple sets in different subplots.

nq.plot(x1, [y1, y2, y3], ylabel='Label', together=False, title='Same label for all y axes')
nq.plot(x1, [y1, y2, y3], ylabel=['Label 1','Label 2', 'Label 3'], together=False, title='Different labels for each y')

# Logarithmig scales ---------------------------------------
nq.plot(x1, [y1, y2, y3], xscale='L', xlabel='Log scale')
nq.plot(x1, [y1, y2, y3], together=False, yscale='L', ylabel='Log scale')
nq.plot(x1, [y1, y2, y3], together=False, yscale='llL', ylabel=['Lin scale', 'Lin scale', 'Log scale'])
I_want_log_axes = nq.plot(x1, [y1, y2, y3], together=False, ylabel=['Lin scale', 'Log scale', 'Lin scale'])
