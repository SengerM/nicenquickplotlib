...
# Changing the figstyle -------------------
nq.set_figstyle('default')
nq.plot(x, [np.sin(x[0]), np.cos(x[1]), np.sqrt(x[2])], legend=[r'$\sin(x)$', r'$\cos(x)$', r'$\sqrt{x}$'], marker=True, title='markers default in default figstyle')
nq.plot(x, [np.sin(x[0]), np.cos(x[1]), np.sqrt(x[2])], legend=[r'$\sin(x)$', r'$\cos(x)$', r'$\sqrt{x}$'], marker=['+','x','o'], title='markers specified by a list in default figstyle')
