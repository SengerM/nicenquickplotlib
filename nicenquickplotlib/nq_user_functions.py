...
def plot_yerr(x, y=None, xlabel=None, ylabel=None, legend=None, title=None, 
	...
	x : numpy array
		...
		If 'y' data is providen then:
			...
			2) If 'x' is a list of numpy arrays each is used with each
			'y' data set.
	y : unumpy (uncertainties package) array.
		y values to plot. 
	xlabel : string, optional
		String containing the label for the x axis.
	ylabel : string or list of strings, optional
	...
	# Plot -------------------------------------------------------------
	for k in range(len(yy)):
		x_data = np.array(xx[k])
		y_vals = np.array(unp.nominal_values(yy[k]))
		y_errs = np.array(unp.std_devs(yy[k]))
		axes[k].fill_between(x_data, y_vals-y_errs, y_vals+y_errs, color=colors[k%len(__figstyle.colors)], alpha=alpha)
		axes[k].plot(x_data, y_vals, color=colors[k%len(__figstyle.colors)], linestyle=linestyles[k%len(linestyles)], marker=markers[k%len(markers)], *args, **kwargs)
		__figstyle.grid(axes[k])
		# Configure legend ---------------------------------------------
		if legend is not None:
			if isinstance(legend, str):
